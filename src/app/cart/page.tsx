  ...
  return (
    <div className="w-full bg-brand-bg min-h-screen py-10">
      <div className="mx-auto max-w-7xl px-6">
        <h1 className="text-4xl font-serif text-brand-maroon mb-2">{t('cart_title')}</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="w-full lg:w-2/3">
            <div className="bg-white rounded-xl shadow-sm border border-black/5 overflow-hidden">
              <div className="hidden md:grid grid-cols-12 gap-4 p-6 border-b border-black/5 text-xs font-bold uppercase tracking-widest text-brand-text-light">
                <div className="col-span-6">{t('nav_home')} (Product)</div>
              ...
              <div className="divide-y divide-black/5">
                {items.length === 0 ? (
                  <div className="p-12 text-center text-brand-text-light">
                    <p className="mb-4">{t('cart_empty')}</p>
                    ...
                    <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-6 items-center relative">
                      <div className="col-span-1 md:col-span-6 flex gap-4">
                        ...
                        <div className="flex flex-col justify-center">
                          ...
                          {item.size && <div className="text-xs text-brand-text-light mt-1">{t('size')}: {item.size}</div>}
                          {item.color && <div className="text-xs text-brand-text-light">{t('color')}: {item.color}</div>}
                        </div>
                      </div>
          ...
          <div className="w-full lg:w-1/3">
            <div className="bg-white rounded-xl shadow-sm border border-black/5 p-6 sticky top-24">
              <h2 className="text-xl font-serif text-brand-maroon mb-6">{t('order_summary')}</h2>
              ...
              <div className="flex justify-between items-end mb-8">
                <span className="font-bold text-brand-text">{t('total')}</span>
